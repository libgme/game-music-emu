...
public:
	...
	struct spc_block_t
	{
		...
		byte channel_disable;      // channel enable bits (0 = enabled, 1 = disabled)
		byte date [4];             // BCD: MM DD YY YY (decoded as YYYY/MM/DD)
...
private:
	...
	long file_size;
	int spc_count_;
	long ram_blocks_count_;     // number of complete 256-byte RAM blocks following the SPC blocks area
	int track_channel_disable_; // mute bits from current SPC block (combined with user mute)

	Fir_Resampler<24> resampler;
	SPC_Filter filter;
	Snes_Spc apu;

	// In-memory SPC file image fed to apu.load_spc(). Member rather than static so
	// concurrent start_track_() calls on different instances do not corrupt each other.
	byte spc_buffer_ [Snes_Spc::spc_file_size];

	spc_block_t const* get_spc_block( int track ) const;
	byte const* get_ram_blocks() const;
	long ram_blocks_offset() const;
	blargg_err_t reconstruct_ram_into( spc_block_t const* sb, byte* ram_out ) const;
