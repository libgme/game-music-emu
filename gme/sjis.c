...
static bool check_utf8(const unsigned char *in, size_t in_len) {
  char cur;
  int bytes_after = 0;
...
int convert_sjis(const unsigned char *in, size_t in_len, unsigned char *out, size_t o_len) {
  if (check_utf8(in, in_len))
    return 0;
  ...
  unsigned char obuf[o_len];
  int len = 0;

  for (size_t i = 0; i < in_len; i ++) {
    buf[cnt] = *(in + i);
    retval = sjis_mbtowc(&pwc, buf, cnt + 1);

    switch (retval){
      case 2:
        len = utf8_wctomb(&mb[0], pwc);
        cnt = 0;
        pwc = 0;

        memcpy(&obuf[out_cnt], mb, len);
        out_cnt += len;
        break;
      case 1:
        obuf[out_cnt] = (unsigned char) pwc;
        out_cnt++;
        cnt = 0;
        pwc = 0;
        break;
