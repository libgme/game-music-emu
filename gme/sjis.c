...
static int jisx0201_mbtowc (int *pwc, const unsigned char *s) {
  unsigned char c = *s;
  if (c < 0x80) {
    if (c == 0x5c)
      *pwc = (int) 0x00a5;
...
static int jisx0208_mbtowc (int *pwc, const unsigned char *s, size_t n) {
...
static int sjis_mbtowc (int *pwc, const unsigned char *s, size_t n) {
  unsigned char c = *s;
  if (c < 0x80 || (c >= 0xa1 && c <= 0xdf))
    return jisx0201_mbtowc(pwc,s);
  else {
    unsigned char s1, s2;
    s1 = c;
...
static int utf8_wctomb(unsigned char *r, int wc) {
  size_t count;
  if (wc < 0x80)
    count = 1;
  else if (wc < 0x800)
...
/* See the first table:
 * https://www.unicode.org/mail-arch/unicode-ml/y2003-m02/att-0467/01-The_Algorithm_to_Valide_an_UTF-8_String */

static bool check_utf8(const unsigned char *in) {
  char cur = *in;
  int bytes_after = 0;
  while (cur != 0) {
    cur = *(in++);
...
int convert_sjis(unsigned char *conv) {
  if (check_utf8(conv))
    return 0;

  int retval;
  unsigned char *dest = conv;

  size_t multibyte_cnt = 0;
  unsigned char multibyte_sjis[2] = {0};
  ...
  int multibyte_len = 0;
  unsigned char multibyte_utf8[4];

  while (*conv != 0){
    multibyte_sjis[multibyte_cnt] = *(conv++);
    retval = sjis_mbtowc(&widechar, multibyte_sjis, multibyte_cnt + 1);

    switch (retval){
      case 2:
        multibyte_len = utf8_wctomb(&multibyte_utf8[0], widechar);
        memcpy(&out_buf[out_cnt], multibyte_utf8, multibyte_len);
        out_cnt += multibyte_len;
        multibyte_cnt = 0;
        widechar = 0;
        break;
      case 1:
        out_buf[out_cnt] = (unsigned char) widechar;
        out_cnt++;
        multibyte_cnt = 0;
        widechar = 0;
        break;
